class FlightsController < ApplicationController
  ...
  def index
    if params[:sort] != "price"
      ...
      @flights = Flight.all.sort_by{|flight| flight.price}
    else
      @flights = Flight.all
    end
  end
  ...
  def search
    if @flights = Flight.all.find{|flight| airline.name.include?(params[:search])}
  ...
  private
    ...
    def flight_params
      params.require(:flight).permit(:airline, :departure, :arrival, :duration, :price)
    end
end
